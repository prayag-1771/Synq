 ...
 *
 * NOTE: This file is mirrored in `synq-web/src/lib/gitRefParser.ts`. Direct
 * messages are end-to-end encrypted, so the *client* is the only place that can
 * see plaintext and therefore the only place that can parse. The server keeps a
 * copy for agent tooling and webhook enrichment, where content is server-owned.
 */
