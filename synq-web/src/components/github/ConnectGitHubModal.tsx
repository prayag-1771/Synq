...
export default function ConnectGitHubModal() {
  ...
  return (
    <div className="fixed inset-0 z-[60] flex items-center justify-center p-4 bg-canvas/80 backdrop-blur-sm">
      <div className="bg-surface border border-line rounded-2xl w-full max-w-lg shadow-2xl animate-in fade-in zoom-in-95 duration-200">
        <div className="flex items-center gap-3 p-5 border-b border-line/60">
          ...
          <button onClick={close} className="p-2 text-muted hover:text-ink hover:bg-raised rounded-xl transition-colors">
            <X className="w-5 h-5" />
          </button>
        </div>
        ...
        {status?.connected ? (
          <div className="p-5 space-y-4">
            <div className="flex items-center gap-3 p-4 rounded-xl bg-emerald-500/5 border border-emerald-500/20">
              {status.avatarUrl && <img src={status.avatarUrl} alt="" className="w-10 h-10 rounded-lg bg-raised" />}
              <div className="flex-1 min-w-0">
                <div className="text-sm font-medium text-ink">{status.login}</div>
          ...
          <div className="p-5 space-y-4">
            {status?.oauthAvailable && (
              <>
                <button
                  onClick={handleOAuth}
                  disabled={busy}
                  className="w-full flex items-center justify-center gap-2.5 px-4 py-3 rounded-xl bg-ink hover:bg-white text-canvas text-sm font-semibold transition-colors disabled:opacity-50"
                >
            ...
            <form onSubmit={handleConnect} className="space-y-3">
              <div>
                <label className="block text-xs font-medium text-muted mb-1.5">Personal access token</label>
                <div className="relative flex items-center">
                  <KeyRound className="absolute left-3 w-4 h-4 text-subtle" />
                  <input
                    type="password"
                    value={token}
                    ...
                    autoComplete="off"
                    className="w-full pl-10 pr-3 py-2.5 bg-canvas border border-line rounded-xl text-sm text-ink placeholder-faint font-mono focus:outline-none focus:ring-2 focus:ring-indigo-500/40"
                  />
                </div>
              </div>
              ...
              >
                ...
                Create a token with <code className="font-mono text-muted">repo</code> scope
              </a>
              ...
              <button
                ...
                className="w-full flex items-center justify-center gap-2 px-4 py-2.5 rounded-xl bg-indigo-600 hover:bg-indigo-500 disabled:bg-raised disabled:text-faint text-white text-sm font-semibold transition-colors"
              >
