...
export default function CodeEditModal() {
  ...
  return (
    <div className="fixed inset-0 z-[60] flex items-center justify-center p-4 bg-slate-950/80 backdrop-blur-sm">
      <div className="bg-slate-900 border border-slate-800 rounded-2xl w-full max-w-5xl h-[90vh] shadow-2xl flex flex-col animate-in fade-in zoom-in-95 duration-200 overflow-hidden">
        {/* Header */}
        <div className="px-5 py-3.5 border-b border-slate-800/60 flex items-center gap-3">
          ...
          <div className="flex-1 min-w-0">
            <div className="font-mono text-xs text-slate-200 truncate">{target.path}</div>
          ...
          <div className="relative shrink-0">
            <button
              ...
              className="flex items-center gap-1.5 px-2.5 py-1.5 rounded-lg bg-slate-800 hover:bg-slate-700 border border-slate-700/60 text-[11px] font-mono text-slate-300 transition-colors"
            >
            ...
            {showBranchMenu && (
              <div className="absolute top-full right-0 mt-1 w-56 max-h-64 overflow-y-auto bg-slate-900 border border-slate-800 rounded-xl shadow-2xl p-1 z-20 custom-scrollbar">
                {branches.map((b) => (
                  <button
                    key={b.name}
                    onClick={() => {
                    ...
                    className={`w-full text-left px-3 py-1.5 rounded-lg text-[11px] font-mono transition-colors ${
                      b.name === branch ? 'bg-indigo-600/20 text-indigo-300' : 'text-slate-300 hover:bg-slate-800'
                    }`}
                  >
                    {b.name}
          ...
          <button
            ...
            className="p-2 rounded-xl text-slate-400 hover:text-indigo-300 hover:bg-indigo-500/10 transition-colors shrink-0"
          >
        ...
        {/* AI instruction bar */}
        <div className="px-5 py-3 border-b border-slate-800/60 bg-indigo-950/10">
          <div className="flex gap-2">
            <div className="flex-1 relative flex items-center">
              ...
              <input
                ...
                className="w-full pl-9 pr-3 py-2 bg-slate-950 border border-slate-800 rounded-lg text-xs text-slate-200 placeholder-slate-600 focus:outline-none focus:ring-2 focus:ring-indigo-500/40"
              />
            </div>
            <button
              onClick={askAi}
          ...
          {proposal && (
            <div className="mt-3 p-3 rounded-lg border border-indigo-500/30 bg-indigo-950/30">
              <div className="flex items-start gap-2">
                ...
                <p className="flex-1 text-[11px] text-slate-300 leading-relaxed">{proposal.explanation}</p>
              </div>
              <div className="flex items-center gap-2 mt-2.5">
                ...
                <span className="text-[10px] text-slate-600 ml-auto">Nothing is committed until you say so.</span>
              </div>
            </div>
          )}
        </div>
        ...
        <div className="flex-1 min-h-0 relative bg-slate-950">
          {loading ? (
            ...
            <div className="h-full flex font-mono text-[12px] leading-[1.6]">
              <div
                ref={gutterRef}
                className="shrink-0 overflow-hidden bg-slate-900/40 border-r border-slate-800 py-3 text-right select-none"
                style={{ width: `${String(lineCount).length + 3}ch` }}
              >
                {Array.from({ length: lineCount }, (_, i) => (
                  <div key={i} className="px-2 text-slate-600">
                    {i + 1}
                  </div>
                ))}
              </div>
              ...
              <textarea
                ...
                onScroll={syncGutter}
                spellCheck={false}
                className="flex-1 px-3 py-3 bg-transparent text-slate-300 resize-none focus:outline-none leading-[1.6] whitespace-pre"
                style={{ tabSize: 2 }}
              />
            </div>
          )}
              ...
              <>
                <div className="flex gap-2">
                  ...
                  <button
                    ...
                    className="flex items-center gap-1.5 px-3 py-2 rounded-lg border border-slate-800 hover:border-indigo-500/40 text-slate-400 hover:text-indigo-300 text-xs transition-colors disabled:opacity-40"
                  >
                ...
                <div className="flex items-center gap-2 flex-wrap">
                  <label className="flex items-center gap-2 cursor-pointer">
                    <button
                      ...
                      className={`w-4 h-4 rounded border flex items-center justify-center transition-colors ${
                        openPr ? 'bg-indigo-600 border-indigo-500' : 'bg-slate-950 border-slate-700'
                      }`}
                    >
                      {openPr && <Check className="w-3 h-3 text-white" />}
                    </button>
                    <span className="text-[11px] text-slate-400">
                      Open a pull request instead of committing to <span className="font-mono text-slate-300">{branch}</span>
                    </span>
                  </label>
                  ...
                  {isDirty && (
                    <button
                      ...
                      className="flex items-center gap-1.5 px-2.5 py-1.5 rounded-lg text-slate-500 hover:text-slate-300 hover:bg-slate-800 text-[11px] transition-colors"
                    >
                  ...
                  <button
                    ...
                    className="ml-auto flex items-center gap-1.5 px-4 py-2 rounded-xl bg-indigo-600 hover:bg-indigo-500 disabled:bg-slate-800 disabled:text-slate-600 text-white text-xs font-semibold transition-colors"
                  >
                    {committing ? (
        ...
        {toast && (
          <div className="absolute bottom-24 left-1/2 -translate-x-1/2 px-4 py-2 rounded-xl bg-slate-800 border border-slate-700 text-xs text-slate-200 shadow-2xl animate-in fade-in slide-in-from-bottom-2">
            {toast}
          </div>
        )}
      </div>
