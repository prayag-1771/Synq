...
export default function LinkRepoModal({ chatId }: Props) {
  ...
  return (
    <div className="fixed inset-0 z-[60] flex items-center justify-center p-4 bg-canvas/80 backdrop-blur-sm">
      <div className="bg-surface border border-line rounded-2xl w-full max-w-2xl h-[80vh] shadow-2xl flex flex-col animate-in fade-in zoom-in-95 duration-200">
        <div className="flex items-center gap-3 p-5 border-b border-line/60">
          <div className="w-10 h-10 rounded-xl bg-raised border border-line-strong/60 flex items-center justify-center text-ink">
            <GitHubMark className="w-5 h-5" />
          </div>
          <div className="flex-1 min-w-0">
            <h2 className="text-base font-semibold text-ink">Link a repository</h2>
            <p className="text-xs text-subtle mt-0.5">
              Makes <code className="font-mono text-muted">#123</code> and{' '}
              <code className="font-mono text-muted">src/file.ts:20-40</code> resolve in this conversation
            </p>
          </div>
          <button
            onClick={() => openLinkRepoModal(false)}
            className="p-2 text-muted hover:text-ink hover:bg-raised rounded-xl transition-colors"
          >
            <X className="w-5 h-5" />
          </button>
        </div>
        ...
        <div className="p-4 border-b border-line/60 space-y-3">
          <div className="relative flex items-center">
            <Search className="absolute left-3.5 w-4 h-4 text-subtle" />
            <input
              autoFocus
              value={query}
              onChange={(e) => setQuery(e.target.value)}
              placeholder="Filter repositories…"
              className="w-full pl-10 pr-3 py-2.5 bg-canvas border border-line rounded-xl text-sm text-ink placeholder-faint focus:outline-none focus:ring-2 focus:ring-indigo-500/40"
            />
          </div>
          ...
          <label className="flex items-start gap-2.5 cursor-pointer group">
            <div
              className={`mt-0.5 w-4 h-4 rounded border flex items-center justify-center transition-colors ${
                installWebhook ? 'bg-indigo-600 border-indigo-500' : 'bg-canvas border-line-strong group-hover:border-line-strong'
              }`}
              onClick={(e) => {
                e.preventDefault();
            ...
            <div className="flex-1">
              <div className="text-xs font-medium text-ink/85 flex items-center gap-1.5">
          ...
          {loading ? (
            ...
            filtered.map((repo) => (
              <button
                ...
                className="w-full text-left p-3 rounded-xl border border-transparent hover:border-line hover:bg-raised/40 transition-colors disabled:opacity-50 group"
              >
                <div className="flex items-center gap-2.5">
                  ...
                  <span className="text-sm font-medium text-ink group-hover:text-white truncate">
                    {repo.fullName}
                  </span>
                  {repo.language && (
                    <span className="text-[10px] px-1.5 py-0.5 rounded-full bg-raised text-muted shrink-0">
                      {repo.language}
                    </span>
                  )}
                  {repo.stars > 0 && (
                    <span className="flex items-center gap-0.5 text-[10px] text-subtle shrink-0">
                ...
                {repo.description && (
                  <p className="text-[11px] text-subtle mt-1 line-clamp-1 pl-6">{repo.description}</p>
                )}
              </button>
            ))
          )}
